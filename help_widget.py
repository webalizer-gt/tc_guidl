...
class HelpWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.help_label = QLabel("Help Information", self)
        self.help_label.setAlignment(Qt.AlignTop | Qt.AlignHCenter)
        self.help_label.setStyleSheet("font-size: 24px; font-weight: bold; color: white;")
        self.layout.addWidget(self.help_label)

        image_path = self.resource_path("assets/images/derp.webp")
        self.html_viewer = QTextBrowser(self)
        self.html_viewer.setStyleSheet("color: white; padding: 3px;")
        self.html_viewer.setHtml(f"""
        <h1>tc_guidl</h1>
...
""")
        self.html_viewer.setOpenLinks(False)  # Disable all link handling by QTextBrowser
        self.html_viewer.anchorClicked.connect(self.handle_anchor_click)
        self.layout.addWidget(self.html_viewer)

        self.status_update.emit("Help widget loaded")

    def handle_anchor_click(self, url):
        if url.scheme() in ("http", "https"):  # Check if it's an external link
            QDesktopServices.openUrl(url)
        else:  # Handle internal anchors
            self.html_viewer.scrollToAnchor(url.fragment())
